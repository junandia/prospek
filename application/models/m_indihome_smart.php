...
class M_indihome_smart extends CI_Model {
    ...
    Public function indihome_smart($periode_awal, $periode_akhir) {
    	if ($periode_awal==null||$periode_akhir==null){
    		...
    		$wordsquery = "SELECT WITEL, COUNT(*) as jtb FROM indihome WHERE NOT EXISTS (SELECT * from indihome_smart WHERE indihome.NO_INET = indihome_smart.NO_INET) AND TGL_REG BETWEEN '$periode_awal' AND '$periode_akhir' GROUP BY WITEL";
    	}
    ...
    public function chek_duplicat($id){
        $QuerySql = $this->db->query("SELECT no_inet FROM indihome_smart WHERE no_inet IN ('$id')");
        return $QuerySql->row();
    }

    public function update_duplicat($witel, $ncli, $ndos, $ndem, $no_inet, $item, $price, $tgl_va, $tgl_ps, $kcontact) {
        $this->db->query('UPDATE indihome_smart SET witel = "'.$witel.'",
                                             ncli = "'.$ncli.'", 
                                             ndos = "'.$ndos.'", 
                                             ndem = "'.$ndem.'", 
                                             no_inet = "'.$no_inet.'", 
    ...
    {
        $this->db->query('REPLACE INTO indihome_smart (witel, ncli, ndos, ndem, no_inet, item, price, tgl_va, tgl_ps, kcontact)
            VALUES("'.$witel.'", 
                    "'.$ncli.'",
                     "'.$ndos.'", 
                     "'.$ndem.'", 
